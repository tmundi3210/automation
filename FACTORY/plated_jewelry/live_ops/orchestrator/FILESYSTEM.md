...
> **Authority.** This file is the single authority for the orchestrator's on-disk state. `live_ops/orchestrator/ORCHESTRATOR.md §2` sketches a working tree and says *"FILESYSTEM.md is the authority; follow it if it differs."* It differs — **use the `work/` tree defined here.** A name map from the ORCHESTRATOR.md sketch to this canonical tree is at the bottom (§7).
>
...
└── work/                                   ← THE STATE LAYER (created first boot; entire mutable surface)
    ...
    ├── signals/
    │   └── YYYY-MM-DD--<lane>--<n>.json     ← append-only dated SIGNAL RECORD batches (a JSON array per file)
    ├── analysis/
...
| `work/signals/YYYY-MM-DD--<lane>--<n>.json` | One tick's harvest from one lane: an **array** of SIGNAL RECORDS, one object per observation. `<lane>` ∈ `live｜history`; `<n>` is a zero-padded batch index (`00`,`01`,…) that increments per additional collection in the same date+lane. | **SIGNAL RECORD** (array) | LIVE_RESEARCH / HISTORY agents (step a) | ANALYST (step c), SAFETY_REVIEW (step b) — **by path** | Append-only: a **new file per batch**; existing batch files are never edited |
...
| `work/secrets.env.example` | **Named slots only, no values** — the env-var names for the four API slots (`SEARCH_API_KEY`, `LLM_API_KEY`, `POLL_HOST_API_KEY`, `RESPONSE_CAPTURE_API_KEY`), mirroring `config.json.api_slots`. A template the owner copies to `work/secrets.env` and fills. | (env template; not a record) | Orchestrator (first boot) | Owner | Written once |
...
6. **SAFETY_REVIEW gates every write into `signals/`.** No SIGNAL RECORD reaches `signals/` until the SAFETY_REVIEW agent has scanned it. Flagged records go to `quarantine/` with a reason and `security_flag:true`; they are never promoted. Every live-lane record carries `security_flag` (see OUTPUT_FORMAT.md validation).
...
3. Write `secrets.env.example` with the four named slots (§2), and ensure `work/secrets.env` is gitignored.
...
5. Run the one-time **history** baseline (weak lane) → first `signals/YYYY-MM-DD--history--00.json`, so the first live tick has a prior to compare against.
...
## 7. NAME MAP — ORCHESTRATOR.md §2 sketch → this canonical tree

The sketch in `ORCHESTRATOR.md §2` predates this authority file. Equivalences (use the right-hand column):

| ORCHESTRATOR.md §2 sketch | Canonical (this file) |
|---|---|
| `state.json` | `work/state.json` |
| `records/signals/<YYYY-MM-DD>/` (per-observation files) | `work/signals/YYYY-MM-DD--<lane>--<n>.json` (per-batch arrays) |
| `records/analysis/analysis_<YYYY-MM-DD>.json` | `work/analysis/YYYY-MM-DD--rollup.json` (+ `work/analysis/latest.json`) |
| `records/safety/gate_<YYYY-MM-DD>.json` | `work/quarantine/YYYY-MM-DD--<id>.json` |
| `poll/` | `work/poll/poll_spec.json` + `work/poll/poll.html` |
| `log/tick_<YYYY-MM-DD>.md` | `work/logs/YYYY-MM-DD.md` |
