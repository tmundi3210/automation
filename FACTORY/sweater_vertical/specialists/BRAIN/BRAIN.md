...
## The self-improvement loop (T12)

The brain also carries a **reflection loop** that keeps the specialists honest over time. It makes any one of the nine specialists interrogate **its own analysis** in **fresh agent contexts** (the base unit is a fresh context window, so the specialist can't just agree with itself): **R1 questioner** (adversary) → **R2 answerer** (the specialist itself, blind to R1's reasoning) → **R3 judge** (neutral) → **R4 synthesizer**, dry-stopping when a round finds nothing material (max 2 rounds). It can apply only high-confidence, in-scope, **mechanical** fixes that still pass `build.sh`; anything needing new measurement or judgment becomes a backlog item, and a fix that would require an invented fact is refused and logged as a gap. Protocol: `self_improvement_loop.json` (machine) + `SELF_LOOP.md` (human doc + a paste-prompt to run one manual round). Outputs live under `analysis/self_loop/` (`SELF_LOOP_SUMMARY.md`, `APPLIED.md`, `PROPOSED_BACKLOG.md`, and per-specialist `LOOP_REPORT.md`/`feedback.json`).

**First full run (2026-07-01, all nine specialists):** 9/9 re-gated **ALL GREEN** after fixes; **zero fabrication failures** brain-wide (the three governing blockers stayed `[UNKNOWN]`, the off-paper economy stayed mechanism-not-accusation, the corrected duty trio held). It caught one real content-level HIGH — yarn's named-firm "USTER-class lab QC" over-attribution — now demoted to `[ESTIMATE: industry-lore]` / `[UNKNOWN]` at the **KB layer** so a re-grounding can't re-import it. The dominant weakness family was **under-referencing**: correct guardrails (the GATED-conditional-GO posture, the four gates, the pilling-floor ownership) living in shared context rather than being written into the standalone specialist file that would load alone — now inlined for the load-bearing cases. See `analysis/self_loop/APPLIED.md` for the per-file record.
