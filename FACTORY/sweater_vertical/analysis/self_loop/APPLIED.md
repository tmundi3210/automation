...
|---|---|---|
| **yarn** | `yarn.specialist.json`, `kb1_yarn_fibre_line.spec.json` + `.kb.json`, `kb2_yarn_sourcing_supply.spec.json` + `.kb.json` | **Content HIGH (F1).** The named integrated spinners (Vardhman/Sportking/Nahar/Oswal) no longer carry "in-house USTER-class lab QC" as flat named-firm fact. Now a three-way honest split at the **KB layer** (so a re-grounding cannot re-import it): the roster is `[FACT]`; "large integrated spinners **as a class** run USTER-class lab QC" is `[ESTIMATE: industry-lore, MED-HIGH]`; whether any **specific** named firm runs such a lab and will certify our small clause-bound lot is `[UNKNOWN]` until a live quote/audit (else budget a third-party spot-check). **Duty HIGH (F2)** — _propagated to the KB layer in a follow-up._ The old "defer ALL duty to T9" language collapsed the **India-side raw-fibre import duty** (imported merino/superfine tops + cashmere entering India for the Ludhiana spin — a real A3 imported-fibre cost layer) into the **US-side 6110 finished-garment duty**. Now de-conflated in kb1 + kb2 exclusions and grounded in kb2's `PREMIUM_FIBRE_IMPORT_ROUTING` node (definition + failure_mode + revisit_trigger): the India-side layer is held `[UNKNOWN]`, mapped to DECISION_MEMO **G5** (forward India duty), computation still deferred to T9, **no India duty rate invented**. (The finalizer had applied F2 only to the specialist file; this pushes it down into the grounded KB specs so a re-distill can't lose it.) |
