...
> **ERRATUM (added after the build wave) — read this before the caveats below.** When this dive was first written, the independent make-vs-buy analyst had **timed out** and `analyst_b_make_vs_buy.md` did not exist — so the original text (and several notes below) called the make-vs-buy verdict "single-analyst / un-cross-examined / [FACT — file absent]." **That is now superseded.** The adversarial analyst-B was **subsequently written** (a full B1/B2/B3 make-vs-buy stress test with four honest counters and explicit flip-conditions), and the T4 specialist + KBs now carry a **tested A/B conclusion**. Read every "analyst-B absent / single-analyst / un-cross-examined / missing-second-analyst / [FACT — file absent]" note below as **corrected to: the verdict was independently cross-examined.** The band-width *downgrades* remain fully valid — but because the **[UNKNOWN] knit+LINK job-work rate** blocks precision, **not** because the verdict lacked review.
>
> **Original process caveat (now resolved).** This segment's per-garment cost model (`analyst_a_cost_model.md`) was written first; the make-vs-buy verdict began as **reconstructed-consensus from T1/T2/T3** and was then **cross-examined by `analyst_b_make_vs_buy.md`** (see §2). Treat §2's verdict as a tested A/B conclusion; treat §3's bands as wide because the knit+LINK rate is [UNKNOWN].
...
1. **Not landed cost.** Ocean freight, US duty (cotton 6110.20.20 ≈16.5% / wool 6110.11 ~16% / acrylic-MMF 6110.30.30 ~32% `[FACT — USITC]`), insurance, broker, drayage, financing all live in **T9**.
...
- **A1 (cotton-blend core) is the everyday seller**; its duty (cotton 6110.20.20 ~16.5%) is **comparable to wool** (~16%) and **far below acrylic/MMF** (6110.30.30 ~32%), so it is the core on demand/sell-window/no-prickle/repeat-LTV grounds — **not on a duty advantage over wool** — and its ex-factory and landed stories agree in T9. (The acrylic A2 article is the one penalized at the border at 32%.)
- **A3 (merino/lambswool premium) is the LEAST reliable band** — it's **yarn-dominated and order-of-magnitude** (the premium fibre price is a single stale anchor; **needs an importer quote before any margin use**), and its high ex-factory yarn cost (not a duty gap - wool ~16% ≈ cotton ~16.5%) makes its *landed* premium wider still. Treat A3's number as a placeholder until an importer quotes the tops.
...
3. **Ex-factory → landed multiplies, it doesn't add a little.** T9 stacks freight + duty (cotton ~16.5% ≈ wool ~16%, with acrylic/MMF ~32% the outlier) + insurance + broker + drayage + financing on top, and **duty is a percentage, so it scales with the ex-factory level** — which is why A3's premium fibre price compounds at landed, and why the acrylic A2 (32% MMF duty) is the article most penalized at the border, while the A1 cotton core stays the structurally friendliest into the US on its demand/sell-window/repeat-LTV strengths, not on a duty edge over wool.
...
12. **~~No independent challenge to the bands or the make-vs-buy boundary.~~ RESOLVED (see Erratum).** `analyst_b_make_vs_buy.md` was subsequently written; the make-vs-buy verdict is now a **tested A/B conclusion** (B1/B2/B3 trade study + four counters + explicit flip-conditions, folded into KB2). The bands stay wide for the right reason — the **[UNKNOWN] knit+LINK rate** — not for lack of review.
...
- **Input status (corrected post-build):** `mfg_economics/analyst_b_make_vs_buy.md` was absent at first-write but was **subsequently written** in the build wave; the make-vs-buy verdict is now **cross-examined (tested A/B)**, not reconstructed-only. See the Erratum at the top.
