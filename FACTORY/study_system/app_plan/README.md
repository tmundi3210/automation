...
| `SPECIALIST_LIST.md` | **The list you asked for**: A) the 9 existing specialists, B) reused loop doctrine, C) the 5 new specialists (`kgraph`, `viz`, `extint`, `media`, `uxguide`) — **now BUILT & gated** at `specialists/<code>/` (3 dense KBs + distilled specialist each, `build.sh` ALL GREEN). |
...
## Status
Planning complete; adversarially audited (2 findings fixed). **The 5 proposed specialists are now built and gated** (`specialists/{kgraph,viz,extint,media,uxguide}/`, `build.sh` ALL GREEN, honesty/fidelity-audited — clean). **Awaiting owner direction** on: which phase of `FEATURE_PLAN.md` to start first (the specialists that each phase consults now all exist).
