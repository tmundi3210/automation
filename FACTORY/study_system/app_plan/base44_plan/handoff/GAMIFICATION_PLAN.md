...
- **ELIGIBLE:** INBDE standalone `remember/understand` terminology gates (anatomy/structure naming, drug-class recognition, definition recall — short homogeneous options). [RECONCILED 2026-07-11 per qcraft §8j: the earlier "TOEFL small tasks (Complete-the-Words, Listen-and-Choose)" examples are BARRED by the fence — Complete-the-Words is typed-not-MCQ (fails clause 1), Listen-and-Choose is audio (reading-floor invalid), passage tasks time reading speed. FIRST SHIP = INBDE terminology gates ONLY; TOEFL branch held until a 7b allowlist is curated.]
...
### 8h. Codex verbatim cross-check DONE (2026-07-11) — strong build, 1 P0 + 2 P1 defects
Slice synced to planit (docs/plan/slices/, commit 4741871); Codex ran code-vs-slice. Verdict: **most load-bearing lines PASS** (rest-day streak B1, effort≠mastery split, forecast structurally-never-green, Learned≥0.95 + demote-never-revoke, live entities + paginated batch-joins + cosmetic-only, Phase 3 correctly absent). Evidence grade upgraded: owner-reported → **audit-verified (Codex code read vs slice)**.

**Defects to fix (one batched pass):**
- **P0 — A6.4 maturation ignores non-review cards.** Code takes min-S over review-state cards but SKIPS non-review cards, so a concept with a mature reviewed card + an un-graduated learning/new card still shows "Mature." Slice rule: ANY card not in `review` state ⇒ the concept is capped at seedling (still being acquired = not durable). This is a real "claims durability it doesn't have" honesty bug. FIX: presence of any non-review card forces seedling.
- **P1 — AC3.5 zero-due forecast day** falls into a dim empty box; render an explicit honest "0 due" light state.
- **P1 — G2-b missing-plan day drains a freeze token** (treated as miss). RESOLVED (below).
...
### 8j. Phase 3 gates RESOLVED (graphux + qcraft, 2026-07-11) — drill is BUILDABLE, INBDE-only first ship
- **8a motion → BUILD hand-rolled** (CSS transforms + CSS `linear()` + a minimal requestAnimationFrame loop for the decay only; NO animation library — the drill is one lightweight animation on one experimental surface, not worth the app's first anim dependency). Fallback = Motion One, prototyped only if the hand-roll benchmark janks. Curves: fall = ease-in (≈cubic-bezier(0.4,0,1,1)) over the full window_ms; rise = ease-out landing on a discrete quantized step (start/end keyframes pinned to floor(level_before)/floor(level_after) — the numeric value only ever reads the step, never an intermediate %); decay = session-only stepped drain; reduced-motion = instant-cut to static numeral/ring + step meter (100% of timer info preserved). Compositor-only transforms; degrade polish (splash/particles) before ever janking the essential timer. **Perf verdict = [ESTIMATE-until-prototyped]: measure sustained frame-time on the OWNER'S PHONE (budget = 1000/refreshHz — 16.7ms@60 / 8.3ms@120) during the build; BUILD passes if it holds with zero dropped frames on the timer path.**
- **7a/8e TOEFL penalty → RESOLVED: no wrong-answer penalty in TOEFL 2026** [FACT via multiple 2026-format secondary sources; ETS primary 403-blocked → verbatim deferred; safe default coincides]. So timeout≠wrong / no-shame is exam-consistent. A wrong drill answer = no rise (no deduction); timeout = "Correct — but the drop fell first" + untimed re-see. Framing safe for both exams.
- **8b/8c ESL floor → PROVISIONALLY RESOLVED [ESTIMATE, owner-tunable]:** `window_ms = base_read_floor + option_count × per_option_read_time`, then `max(window_ms, HARD_MIN_FLOOR)`. Start: **base_read_floor=6000ms, per_option_read_time=1800ms, HARD_MIN_FLOOR=4000ms** (grounded at a protective L2 rate ~100wpm/600ms-per-word; Brysbaert 2019). Worked: 4-option ≈ 13,200ms. **The window is the FAIRNESS floor (protects the slow ESL reader); the SPEED pressure comes from the DECAY, not a tight window — never shrink the window for "snappiness."** Calibrate later by logging render-to-first-input latency on the untimed surface (writes nothing), fitting base=intercept/per_option=slope at the 90-95th percentile, then re-tag [ESTIMATE]→[CALIBRATED]. Plus **WCAG 2.2.1: the user can extend or turn OFF the timer** (timer-off = plain untimed MCQ).
- **8d decay_per_sec → owner-open.** The speed-pressure knob. Starting [ESTIMATE] ~0.15-0.2 level/sec (≈ lose one step per ~30s idle on a 20-step tank), session-only, owner-tunable. engage owns the value; graphux animates it.
- **FIRST SHIP = INBDE remember/understand terminology gates ONLY.** TOEFL held: Listen-and-Choose (audio → reading-floor invalid), Complete-the-Words (typed, not MCQ → fails fence clause 1), passage tasks (time reading speed → ESL hazard). No current-2026 TOEFL task is simultaneously MCQ ∧ non-audio ∧ non-passage ∧ short — so the TOEFL branch is empty until a curated 7b allowlist exists.
- **Still ephemeral:** no `game_timed` writer in Phase 3 (owner-gate 8f); the drill writes NOTHING to BKT/Elo/FSRS/CalibrationBin/day-box.
